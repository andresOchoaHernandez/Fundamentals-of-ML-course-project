...
elseif length(init) == 1  % random initialization
    k = init; % ho un numero;
    idx = randsample(n,k); % estraggo da 1 a n, k volte. Sono indici di punti; 
    m = X(:,idx); % mi pesco i punti corrispondenti agli indici;
...
n = size(X,2);
k = size(mu,2);
logRho = zeros(n,k); % valori di Q(h) NON normalizzati;
